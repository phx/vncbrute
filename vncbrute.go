...
func displayProgress(ctx context.Context, current, total *int64, wg *sync.WaitGroup) {
    ...
    // Initialize uiprogress
    uiprogress.Start()
    bar := uiprogress.AddBar(int(*total))

    // Customize the bar to show additional details
    bar.PrependFunc(func(b *uiprogress.Bar) string {
        elapsed := time.Since(startTime)
        percentage := float64(b.Current()) / float64(*total) * 100
        return fmt.Sprintf("Elapsed: %s | Progress: %.2f%%", formatDuration(elapsed), percentage)
    })
    ...
    bar.AppendFunc(func(b *uiprogress.Bar) string {
        ...
        // Calculate remaining time only if some progress has been made
        var remaining time.Duration
        if currentVal > 0 {
            remaining = time.Duration(float64(elapsed) / float64(currentVal) * float64(*total-currentVal))
        } else {
            remaining = 0
        }

        return fmt.Sprintf("Remaining: %s | Attempts: %d/%d", formatDuration(remaining), b.Current(), *total)
    })

    // Update the bar in a separate goroutine
    go func() {
        for {
            select {
            case <-ctx.Done():
            ...
            default:
                ...
                bar.Set(int(atomic.LoadInt64(current)))
